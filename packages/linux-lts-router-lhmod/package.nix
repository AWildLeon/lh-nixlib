...
#   xanmod  https://gitlab.com/xanmod/linux-patches (linux-6.18.y-xanmod)
{
  lib,
  linux_6_18,
  # The nixos kernel module (system/boot/kernel.nix) re-invokes this package's
  # `.override` with `features`/`randstructSeed`/`kernelPatches`; callPackage
  # makes THIS lambda the overridable one, so anything the module forwards must
  # be accepted here and passed straight through to linux_6_18.override.
  ...
}@args:
